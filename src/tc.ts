...
type TCDictTuple = [string, number];
type TCProbTuple = [string, Fraction];
type TCDictType = {
  [key: string]: {
    nodrop: number;
    tcs: TCDictTuple[];
...
function getAtomicTCs(tc: string): TCProbTuple[] {
  ...
  for (var tcTuple of tcObject.tcs) {
    ...
    // if subTC is not defined, it is atomic, copy it over to atomicBreakdown
    if (!TCDICT[subTC]) {
...
console.log(getAtomicTCs("Act 1 (H) H2H B"));
